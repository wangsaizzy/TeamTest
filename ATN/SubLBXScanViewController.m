...
//  Copyright © 2016年 .by wangsai All rights reserved.
//
...
@interface SubLBXScanViewController ()<UITabBarControllerDelegate>
...
- (void)drawBottomItems
{
    ...
    _btnFlash.center = CGPointMake(CGRectGetWidth(_bottomItemsView.frame)*3/4, CGRectGetHeight(_bottomItemsView.frame)*1/3);
     [_btnFlash setImage:[UIImage imageNamed:@"CodeScan.bundle/qrcode_scan_btn_flash_nor"] forState:UIControlStateNormal];
    [_btnFlash addTarget:self action:@selector(openOrCloseFlash) forControlEvents:UIControlEventTouchUpInside];
    
    self.btnPhoto = [[UIButton alloc]init];
    _btnPhoto.bounds = _btnFlash.bounds;
    _btnPhoto.center = CGPointMake(CGRectGetWidth(_bottomItemsView.frame)/4, CGRectGetHeight(_bottomItemsView.frame)*1/3);
...
- (BOOL)tabBarController:(UITabBarController *)tabBarController shouldSelectViewController:(UIViewController *)viewController
{
    ...
    if (![viewController.tabBarItem.title isEqualToString:@""]) {
        //如果用户从TabBarItem点击进去扫描pop回首页
       
         self.tabBarController.tabBar.hidden=YES;
        
        HomeVController *home = [HomeVController new];
        [((UINavigationController *)tabBarController.selectedViewController) popToViewController:home animated:YES];
        ...
        self.navigationController.navigationBarHidden = YES;
            ...
            [((UINavigationController *)tabBarController.selectedViewController) pushViewController:home animated:YES];
